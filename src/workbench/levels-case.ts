...
export const levelsCase: CaseDef<LevelsState> = {
  ...
  render: (state) => {
    ...
    const rows = [
      ...
      { level: "2 · coordinated", onTime: l2.onTime, lateness: fmtTicks(l2.objective?.lateness ?? 0), back: l2.objective ? clock(l2.objective.allReturned) : "—", valid: l2.validation.ok ? "✓ validated" : "✗", waits: `${l2.tasks.reduce((s, t) => s + (t.groundWait ?? 0) + (t.hover ?? 0), 0)} s` },
      { level: "3 · with feedback", onTime: l3.onTime, lateness: fmtTicks(l3.objective?.lateness ?? 0), back: l3.objective ? clock(l3.objective.allReturned) : "—", valid: l3.validation.ok ? "✓ validated" : "✗", waits: `${l3.tasks.reduce((s, t) => s + (t.groundWait ?? 0) + (t.hover ?? 0), 0)} s` },
    ];
    ...
    const summaries: Record<LevelsState["level"], string> = {
      ...
      "2": `The same assignment, every leg planned in space-time against the reservations: valid, ${l2.onTime} of 20 on time, lateness ${fmtTicks(l2.objective?.lateness ?? 0)}. The waits are real now, and drone A is carrying too much.`,
      "3": `Two migrations chosen with every candidate re-evaluated under the pads and the corridor — ${reference.moves.map((m) => m.description.split(" at position")[0]).join("; ")} — and all 20 are on time, everyone back by ${clock(l3.objective!.allReturned)}. ${reference.candidatesEvaluated} candidates were evaluated to find them.`,
    };
    parts.push(`<p class="wb-summary"><strong>Level ${state.level}:</strong> ${esc(summaries[state.level])}</p>`);
    parts.push(table(
      [{ key: "level", label: "level" }, { key: "onTime", label: "on time", align: "right" }, { key: "lateness", label: "lateness", align: "right" }, { key: "back", label: "all back" }, { key: "waits", label: "waits", align: "right" }, { key: "valid", label: "validator" }],
    ...
    if (state.level === "3") {
      parts.push(table(
        ...
        reference.moves.map((m, i) => ({ n: i + 1, move: m.description, before: `lateness ${fmtTicks(m.before.lateness)}, back ${clock(m.before.allReturned)}`, after: `lateness ${fmtTicks(m.after.lateness)}, back ${clock(m.after.allReturned)}` })),
        "The accepted moves, each a full re-evaluation",
      ));
    }
    ...
    return { html: parts.join(""), status: `computed in your browser · three full evaluations of twenty orders · ${ms} ms · engine 0.1 · reference plan re-evaluated, not read` };
  },
};
