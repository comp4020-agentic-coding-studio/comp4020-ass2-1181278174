...
function geometryRun(r: LabRun) {
    ...
    r.tables = [table('routes', `${paths.filter(p => p.status === 'found').length} legal alternatives`, ['Route', 'Path', 'Time (s)'], paths.filter(p => p.status === 'found').map((p, i) => ({ id: 'route' + i, values: [String.fromCharCode(65 + i), p.path!.join(' → '), p.cost!], path: p.path, selection: { kind: 'route', id: 'route' + i } }))), table('connections', 'Inspect a proposed connection', ['Connection', 'Geometry check', 'Building'], proposals, true)];
...
function labelsRun(r: LabRun, s: Strategies) {
    ...
    r.summary = p.status === 'found' ? `${order.id}: ${p.chosen!.time} s from take-off to landing; ${p.energyUsed} J including service and the unloaded return. ${r.status === 'diagnostic' ? 'The independent reserve check rejects this candidate.' : `${p.energyLeft} J remain on landing.`}` : `${correct.chosen ? 'This strategy lost a feasible route.' : p.status + '.'} ${correct.chosen ? 'The correct Pareto search still finds a complete trip; inspect where this strategy discarded its prefix.' : 'The correct search also finds no complete trip in this domain.'}`;
...
function sequenceRun(r: LabRun, s: Strategies) {
    ...
    const initial = timetable(seq, cost, opts), swaps = improveBySwaps(seq, cost, opts), exact = c.method === 'exact' ? enumerate(jobs, cost, opts) : undefined;
    if (c.method === 'swaps')
        seq = swaps.sequence;
    ...
    r.summary = `Sequence ${seq.map(j => j.id).join(' → ')}. Course objective (lateness, final return, energy): ${objectiveText(result.objective)}. ${exact ? `${exact.count} sequences enumerated with the selected comparison function.` : ''} ${c.method === 'exact' ? 'Exact optimum for this six-job model and selected objective; no claim about larger instances.' : c.method === 'swaps' && swaps.status === 'local-optimum' ? 'Every pair swap was checked; this proves only a local optimum.' : 'This sequence is evaluated without an optimality claim.'}`;
...
function fleetRun(r: LabRun, s: Strategies, progress?: (n: number) => void) {
    ...
    r.scene = { ...sceneFromPlan(world, p), closures: c.week>=10 ? structuredClone(c.scenario.closures) : [] };
    ...
    const occupancies=[...p.occupancies,...(r.scene.closures??[])
        .filter(c=>!p.occupancies.some(o=>o.owner==='closed'&&o.resource==='corridor'&&o.start===c.start&&o.end===c.end))
        .map(c=>({...c,resource:'corridor',owner:'closed',task:undefined}))];
    r.tables.push(table('resources', 'Shared-resource intervals · half-open [start, end)', ['Resource', 'Drone', 'Order', 'Start', 'End'], occupancies.map((o, i) => ({ id: 'resource' + i, values: [o.resource, o.owner, o.task ?? 'closure', o.start, o.end], selection: { kind: 'resource', id: 'resource' + i }, detail: `${o.resource} occupied by ${o.owner} on [${o.start}, ${o.end}).` }))), table('checks', 'Independent full-plan checker', ['Rule', 'Order', 'Evidence'], check.issues.length ? check.issues.map((x, i) => ({ id: 'issue' + i, values: [x.rule, x.order ?? '—', x.detail], tone: 'bad', selection: x.order ? { kind: 'task', id: x.order } : undefined })) : [{ id: 'passed', values: ['All checks', 'All tasks', 'Recomputed directed edges, action times and energy, payload, service, reserve, uniqueness, availability, FCFS pads, corridor intervals and cutoff.'], tone: 'good' }]));
    for (const t of p.tasks) {
    ...
    r.timeline = [...p.tasks.filter(t => t.status === 'flown').map(t => ({ id: t.order, lane: t.drone, start: t.depart!, end: t.land!, label: t.order, kind: t.late ? 'late' : 'task', selection: { kind: 'task' as const, id: t.order } })), ...r.scene.events.filter(e => !['out', 'back', 'service'].includes(e.phase)).map(e => ({ id: e.id, lane: e.drone, start: e.start, end: e.end, label: `${e.order} ${e.phase}`, kind: e.phase, selection: { kind: 'move' as const, id: e.id } })), ...occupancies.map((o, i) => ({ id: 'resource' + i, lane: o.resource, start: o.start, end: o.end, label: `${o.owner} ${o.task ?? 'closure'}`, kind: o.owner==='closed'?'closed':o.resource, selection: { kind: 'resource' as const, id: 'resource' + i } }))];
}
...
function corridorRun(r: LabRun) {
    ...
    r.summary = conflict ? `Conflict on [${Math.max(crossing!.start, window.start)}, ${Math.min(crossing!.end, window.end)}): both drones occupy the corridor.` : c.arrangement === 'wait' ? `A hovers ${wait} s at ${ac.from} and enters when B leaves. Hover energy is charged.` : c.arrangement === 'custom' ? `A waits ${delay} s on the ground at the kitchen, then ${detour ? 'takes the legal detour around the building blocks' : 'flies through the passage between the towers'}. The corridor intervals do not overlap. Ground waiting adds no hover energy.` : 'A takes a legal route around the corridor. Compare the longer flight and its energy with waiting.';
