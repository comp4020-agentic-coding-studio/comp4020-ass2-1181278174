    ...
    async function enable3D() {
        ...
        try {
            ...
            scene = mod.mountScene(host, run.scene, node => select({ kind: 'node', id: node }), {
                ...(new URLSearchParams(location.search).get('lighting') === 'evening' ? { lighting: 'evening' as const, labels: false } : {}),
                onManualNavigation: () => q('[data-action="follow"]')?.setAttribute('aria-pressed', 'false')
            });
            q('[data-map-host]').hidden = true;
            q('[data-camera-tools]').hidden = false;
