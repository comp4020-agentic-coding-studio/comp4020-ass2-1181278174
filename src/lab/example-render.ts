...
export function exampleHtml(r:LabRun,baseline:LabRun,mode='start') {
  ...
  return `<header class="example-heading"><div><span class="lab-eyebrow">WEEK ${String(week).padStart(2,'0')} / THIS WEEK’S DECISION</span><h2>${esc(lessons[week].question)}</h2><p>${esc(exampleIntros[week])}</p></div>${hasMap?button('expand','Expand view','class="example-expand" aria-expanded="false"'):''}</header>
