...
export function exampleHtml(r:LabRun,baseline:LabRun,mode='start') {
  ...
  <div class="example-stage">
    ...
    ${r.trace.length?`<div class="example-trace"><output data-example-trace-count>0 / ${r.trace.length}</output>${button('trace-next','Next search step')}${button('trace-prev','Previous step')}</div>`:''}
