...
export function lessonEvidence(r:LabRun,baseline:LabRun) {
  ...
  if(week===9) {
    ...
    return `<section class="lesson-evidence" data-lesson-shape="corridor-occupancy"><h3>Two towers · one shared passage</h3>${intervalFigure(events.map(e=>({lane:'Drone '+e.drone,start:e.start,end:e.end,label:`${e.start}–${e.end}`,kind:conflict?'conflict':''})),'Whole corridor occupancy intervals')}<p class="lesson-overlap ${conflict?'bad':'good'}">${conflict?`Overlap [${start}, ${end}) · ${end-start} s conflict`:'No overlap · both intervals fit'}</p>${evidenceTable(['Drone','Pass entry','Pass exit'],events.map(e=>[e.drone,e.start+' s',e.end+' s']))}<p>The passage runs between two buildings on the slope. Its capacity-one rule applies to the whole crossing interval, not just the entry time.</p></section>`;
  }
  if(week===10) {
