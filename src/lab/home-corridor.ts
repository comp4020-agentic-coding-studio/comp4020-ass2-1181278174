...
export function homeCorridorHtml(run: LabRun, before: LabRun) {
  ...
  return `<p class="home-corridor-outcome ${clash ? 'conflict' : 'clear'}" data-home-outcome><strong>${clash ? `Conflict · ${clash} seconds shared` : 'Pass check passed · no overlap'}</strong><span>${clash ? `Both drones occupy the ridge gap on [${start}, ${end}).` : a ? 'A enters at the instant B leaves. The extra hover uses energy.' : 'A goes around the ridge. Only B uses the pass.'}</span></p>
    ...
    <figure><figcaption>Who occupies the ridge pass?</figcaption><svg viewBox="0 0 480 166" role="img" aria-label="${a ? `A occupies the pass from ${a.start} to ${a.end} seconds.` : 'A avoids the pass.'} B occupies it from ${b.start} to ${b.end} seconds. ${clash} seconds overlap.">
